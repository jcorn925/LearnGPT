...
export const CareerPathTreeChart = () => {
  ...
  const handleAddChild = (parentId) => {
    const newNodeId = `new_${Math.random().toString(36).substr(2, 9)}`; // Unique ID for the new node
    const newNodes = [...nodes, { id: newNodeId, data: { label: `Node ${newNodeId}`, canAddChildren: true, onAddChild: handleAddChild }, position }];
    const newEdges = [...edges, { id: `${parentId}->${newNodeId}`, source: parentId, target: newNodeId }];
    setNodes(newNodes);
    setEdges(newEdges);
  };
  ...
  const initialNodes = [
    { id: "1", type: 'custom', data: { label: "one", canAddChildren: true, onAddChild: handleAddChild }, position },
  ...
  const CustomNode = ({ id, data }) => {
    return (
      <div className="reactflow__node-default">
        <div>{data.label}</div>
        {data.canAddChildren && (
          <button onClick={() => data.onAddChild(id)}>Add Child</button>
        )}
      </div>
    );
  };
