...
export function useSequenceLoader(sequenceController, treeData, setTreeData) {
  ...
  const treeDataToJSON = async (node) => {
    ...
    const jsonNode = {
      ...
      templates: await getTemplatesForNodeId(node.id),
    ...
    if (node.children) {
      jsonNode.children = node.children.map(treeDataToJSON);
    }
  ...
  const deserializeSequencePath = (sequencePathData) => {
    ...
    if (sequencePathData.sequencePaths) {
      ...
      reader.onload = (event) => {
        ...
        if (mapSequence) {
          ...
          for (const { key, value } of mapSequence) {
            ...
            if (sequence && value.sequencePaths) {
